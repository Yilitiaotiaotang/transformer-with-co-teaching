...
def get_args():
    ...
    parser.add_argument('--model', type=str, default='TransformerM',      # 修改模型 HARTrans TransCNN TransformerM
                        help='model')
...
def load_data(root):
    if root.find("npy") != -1:
        ...
        data = torch.load("data/train_data_with_noise_symmetric_0.50.pt")
...
def c_main():
    ...
    test_data = torch.load('data/validate_data.pt')
    ...
    for epoch in range(n_epochs):
        ...
        if best < acc:
            ...
            torch.save(model, '/home/tbb/THAT/output/{}_noise_symmetric_0.50_20220619.pkl'.format(args.model))
        print("\nBest is", best)
